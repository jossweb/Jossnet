...
/* Connects to a remote server.  Returns the file descriptor or -1
...
int accept_client(int port)
{
    ...
    for (current = result; current != 0 && num_listen < MAX_LISTEN;
        ...
        getnameinfo(current->ai_addr, current->ai_addrlen, host, sizeof(host), serv, sizeof(serv), NI_NUMERICHOST | NI_NUMERICSERV);
        printf("Trying to bind socket fd=%d to %s:%s\n", fd, host, serv);
...
static void print_packet(const char *tag, const uint8_t *packet, size_t len)
{
    size_t index;
    printf("%s:", tag);
...
static int recv_exact_internal(int fd, uint8_t *packet, size_t len)
{
    size_t received = 0;
    while (len > 0) {
...
{
    size_t received = recv_exact_internal(fd, packet, len);
    if (received) {
        if (verbose)
            print_packet("Rx", packet, received);
    } else if (verbose) {
        printf("Rx: EOF\n");
    }
    return received;
...
size_t recv_packet(int fd, uint8_t *packet, size_t max_len)
{
    ...
    if (max_len < 2)
        return 0;
    if (!recv_exact_internal(fd, packet, 2))
        return 0;
    size = (((size_t)(packet[0])) << 8) | ((size_t)(packet[1]));
    if (size > (max_len - 2))
        return 0;
    size = recv_exact_internal(fd, packet + 2, size);
    if (size)
        size += 2;
    if (verbose)
        print_packet("Rx", packet, size);
    return size;
}
