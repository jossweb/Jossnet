...
int gen_keys()
{
    ...
    const KeyFile key_files[] = {
        ...
        {"25519", "keys/server_key_25519", "keys/server_key_25519.pub"},
        {"448",   "keys/client_key_448",   "keys/client_key_448.pub"},
        {"448",   "keys/server_key_448",   "keys/server_key_448.pub"}
    };
    uint8_t *priv_key = 0;
    ...
    for (int i = 0; i < 4; i++) {
        err = noise_dhstate_new_by_name(&dh, key_files[i].algorithm);
        if (err != NOISE_ERROR_NONE) {
            noise_perror(key_files[i].algorithm, err);
            return 0;
        }
        ...
        /* Save the keys */
        if (ok)
            ok = save_private_key(key_files[i].private_key, priv_key, priv_key_len);
        if (ok)
            ok = save_public_key(key_files[i].public_key, pub_key, pub_key_len);
        ...
        if (!ok) {
            unlink(key_files[i].private_key);
            unlink(key_files[i].public_key);
        }
    }
    //generate psk
    if (!gen_psk()) {
